...
import com.agrishakti.dto.JwtResponse;
import com.agrishakti.dto.LoginRequest;
import com.agrishakti.dto.MessageResponse;
import com.agrishakti.dto.SignupRequest;
...
public class AuthController {
    ...
    public ResponseEntity<?> registerUser(@Valid @RequestBody SignupRequest signUpRequest) {
        ...
        // Save farmer payment details if provided
        if (signUpRequest.getUpiId() != null) {
            user.setUpiId(signUpRequest.getUpiId());
        }
        if (signUpRequest.getBankAccountNumber() != null) {
            user.setBankAccountNumber(signUpRequest.getBankAccountNumber());
        }
        if (signUpRequest.getBankIfscCode() != null) {
            user.setBankIfscCode(signUpRequest.getBankIfscCode());
        }
        if (signUpRequest.getBankName() != null) {
    ...
    public ResponseEntity<?> deleteUser(@PathVariable Long id) {
        ...
        // 1. Delete all products listed by this farmer
        productRepository.deleteByFarmer(user);
        
        // 2. Delete the user (this will also remove entries in user_roles table due to many-to-many mapping)
